        ...
        <section id="portfolio" className="section-block">
            <div className="flex flex-col gap-8">
                <div className="flex flex-col gap-2">
                    ...
                    <h2 className="text-[2rem] font-medium tracking-tight text-(--text-color)">What I&apos;m building</h2>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-8">
                    {projects.map((project, index) => (
                        ...
                        >
                            <Link href={project.link} className={`block aspect-video rounded-lg overflow-hidden ${project.image} relative border border-(--border-color) transition-transform duration-300 group-hover:scale-[1.005]`}>
                            ...
                            <div className="flex items-start gap-2 group-hover:text-(--text-color) transition-colors">
                                <p className="text-[12px] text-(--text-muted) leading-relaxed group-hover:text-inherit">
                                    {project.description}
                                </p>
                                <i className="bx bx-right-arrow-alt text-sm mt-0.5 text-(--text-muted) group-hover:text-inherit group-hover:translate-x-1 transition-all"></i>
                            </div>
                        </motion.div>
                    ))}
                </div>
